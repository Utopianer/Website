...
summary: Learn how to fetch ENS name and avatar for any ENS domain using EthersJs.
...
```js [[1, 14, "const provider = new ethers.providers.JsonRpcProvider(URL_TO_RPC_NODE);"], [2, 16, "ensName"], [3, 19, "avatar"]]
  ...
  useEffect(() => {
    const resolveENS = async () => {
      ...
      if (address && ethers.utils.isAddress(address)) {
        try {
          const provider = new ethers.providers.JsonRpcProvider(URL_TO_RPC_NODE);
